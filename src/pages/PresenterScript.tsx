...
const slides: ScriptSlide[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Sarah on Strategy — convergence score ninety-two out of a hundred: every long-range signal she tracks, from health to sustainability to protein anxiety, is pointing the same way. Chloe on Market Sizing — a two-point-one billion dollar TAM growing at eleven percent compound, and Southeast Asia is the white space inside it: Indonesia, Vietnam, the Philippines — none of the global brands have moved yet. Sebastian on Competitive — threat level high: a regional rival is already building manufacturing capacity, four patents filed in the last ninety days, and his alerts caught the supplier shift two weeks before trade press did. Priya on Innovation — three of your five existing concepts already pass against the converging trends, so you don't need a clean-sheet R&D programme; you need to accelerate what's already in the pipeline. Marcus on Commercial — two of three target retailers have a first-mover slot open in their next category review, and his account history says they'll listen if you bring the evidence. And David on Procurement — one contract sits behind every number on this slide, not fourteen, so the answer itself costs less than last quarter's reconciliation bill.",
