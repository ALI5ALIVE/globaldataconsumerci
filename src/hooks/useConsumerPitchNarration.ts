...
import { getConsumerPitchNarration } from "@/data/consumerPitchNarration";
...
export const useConsumerPitchNarration = () => {
  ...
  const fetchAudio = async (slideId: number, forceRegenerate = false): Promise<string> => {
    const narration = getConsumerPitchNarration(slideId);
    if (!narration) throw new Error(`No narration for consumer pitch slide ${slideId}`);
  ...
  const play = useCallback(async (slideId: number, options?: { forceRegenerate?: boolean }) => {
    ...
    try {
      ...
      console.error("Failed to play consumer pitch narration:", error);
