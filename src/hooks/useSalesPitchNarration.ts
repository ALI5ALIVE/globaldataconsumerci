...
import { getSalesPitchNarration } from "@/data/salesPitchNarration";
...
export const useSalesPitchNarration = () => {
  ...
  const fetchAudio = async (slideId: number, forceRegenerate = false): Promise<string> => {
    const narration = getSalesPitchNarration(slideId);
    if (!narration) {
      throw new Error(`No narration found for sales pitch slide ${slideId}`);
    }
  ...
  const play = useCallback(async (slideId: number, options?: { forceRegenerate?: boolean }) => {
    ...
    try {
      ...
      console.error("Failed to play sales pitch narration:", error);
      setState(prev => ({ ...prev, isPlaying: false, isLoading: false }));
    }
  }, [stop]);
